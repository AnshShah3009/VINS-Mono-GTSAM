...
{
public:
    ARDemoNode() : Node("ar_demo")
    {
        this->declare_parameter<bool>("use_undistored_img", false);
        this->declare_parameter<string>("calib_file", "");

        this->get_parameter("use_undistored_img", USE_UNDISTORED_IMG);
        ...
        line_color_r.r = 1.0; line_color_r.a = 1.0;
        line_color_g.g = 1.0; line_color_g.a = 1.0;
        line_color_b.b = 1.0; line_color_b.a = 1.0;

        string calib_file;
        this->get_parameter("calib_file", calib_file);
        RCLCPP_INFO(this->get_logger(), "reading parameter of camera %s", calib_file.c_str());
        m_camera = CameraFactory::instance()->generateCameraFromYamlFile(calib_file);

        object_pub = this->create_publisher<visualization_msgs::msg::MarkerArray>("AR_object", 10);
        ...
        sub_img = this->create_subscription<sensor_msgs::msg::Image>(
            USE_UNDISTORED_IMG ? "image_undistored" : "image_raw", 100, 
            std::bind(&ARDemoNode::img_callback, this, std::placeholders::_1));

        sub_pose = this->create_subscription<nav_msgs::msg::Odometry>(
            "camera_pose", 100, std::bind(&ARDemoNode::pose_callback, this, std::placeholders::_1));

        sub_point = this->create_subscription<sensor_msgs::msg::PointCloud>(
            "pointcloud", 2000, std::bind(&ARDemoNode::point_callback, this, std::placeholders::_1));
...
private:
   void axis_generate(visualization_msgs::msg::Marker &line_list, Vector3d &origin, int id)
    ...
    void draw_object(cv::Mat &AR_image)
    {
        ...
        int index[cube_num];
        for (int i = 0; i < cube_num; i++) index[i] = i;
        ...
        for (int i = 0; i < cube_num; i++)
            for (int j = 0; j < cube_num - i - 1; j++)
            {
                ...
                {
                    swap(Cube_center_depth[j], Cube_center_depth[j+1]);
        ...
        for (int k = 0; k < cube_num; k++)
        {
            ...
            cv::Point p[8];
            for(int j=0; j<8; j++) p[j] = cv::Point(output_Cube[i][j].x(), output_Cube[i][j].y());
    ...
    void process_data(const sensor_msgs::msg::Image::ConstSharedPtr& img_msg, const nav_msgs::msg::Odometry::ConstSharedPtr pose_msg)
    {
        if(img_cnt < 50)
        {
            img_cnt ++;
        ...
        look_ground = (acos(w_cam_z.dot(Vector3d(0, 0, 1))) * 180.0 / M_PI < 90);
        ...
        cv::Mat AR_image = ptr->image.clone();
    ...
    void point_callback(const sensor_msgs::msg::PointCloud::ConstSharedPtr point_msg)
    {
        ...
        int height_range[30] = {0};
        double height_sum[30] = {0};
        ...
        if (tmp_num < (int)point_msg->points.size() / 2) return;
    ...
    void pose_callback(const nav_msgs::msg::Odometry::ConstSharedPtr pose_msg)
    {
        ...
        if (img_buf.empty()) return;

        while (!img_buf.empty() && rclcpp::Time(img_buf.front()->header.stamp).seconds() < rclcpp::Time(pose_msg->header.stamp).seconds())
        {
            img_buf.pop();
        }
        ...
        if (!img_buf.empty())
        {
            process_data(img_buf.front(), pose_msg);
            img_buf.pop();
        }
    }
    ...
    unique_ptr<image_transport::ImageTransport> it;
};
